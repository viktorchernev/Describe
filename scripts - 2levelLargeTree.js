...
function translateProductionShallow(item, isShallow = false) {
	...
	var items = "";
	for (var i = 0; i < item.items.length; i++) {
		items += translateItemShallow(item.items[i], isShallow);
	}
...
function translateItemShallow(item, isShallow = true) {

	if (item.name == "branch") {
		
		if (isShallow == false) {
		
			return translateProductionShallow(item, true);
		}
...
function listreeAdd(element) {
	...
	element.innerHTML = "<span style='cursor:default;' class='plusminus'>+ </span>" + element.innerHTML;
